...
# ... [Your existing code loading the MovieLens ratings into a variable, e.g., `ratings_df`] ...
# Inside train.py
...
if URL and KEY:
    print("Fetching live user data from Supabase...")
    ...
    if response.data:
        custom_ratings = pd.DataFrame(response.data)
        
        # Hash string usernames into unique integer IDs 
        custom_ratings['user_id'] = custom_ratings['username'].apply(
            lambda x: int(hashlib.sha256(x.encode('utf-8')).hexdigest()[:8], 16) % 100000 + 10000
        )
        custom_ratings = custom_ratings[['user_id', 'movie_id', 'rating']]
        
        # Append to the main MovieLens dataframe
        ratings_df = pd.concat([ratings_df, custom_ratings], ignore_index=True)
        print(f"Successfully injected {len(custom_ratings)} cloud ratings!")
# ── NEW: INJECT LIVE USER DATA ──────────────────────────────────────────
CSV_PATH = "data/master_user_interactions.csv"
if os.path.exists(CSV_PATH):
    print(f"Injecting live user data from {CSV_PATH}...")
    try:
        # Load the CSV generated by the Flask app
        custom_ratings = pd.read_csv(CSV_PATH)
        
        # Hash string usernames into unique integer IDs > 10000 
        # This prevents collisions with MovieLens users (IDs 1-6040)
        custom_ratings['user_id'] = custom_ratings['username'].apply(
            lambda x: int(hashlib.sha256(x.encode('utf-8')).hexdigest()[:8], 16) % 100000 + 10000
        )
        
        # Standardize columns to match the MovieLens dataframe format
        # Adjust 'movie_id' or 'rating' if your train.py uses different column names
        custom_ratings = custom_ratings[['user_id', 'movie_id', 'rating']]
        
        # Append the new user data to the existing matrix
        ratings_df = pd.concat([ratings_df, custom_ratings], ignore_index=True)
        print(f"Successfully added {len(custom_ratings)} new user ratings to the training matrix.")
        
    except Exception as e:
        print(f"Warning: Failed to merge user CSV data. Proceeding with base data. Error: {e}")
# ────────────────────────────────────────────────────────────────────────

# ... [The rest of your train.py script (Building rating matrix, TruncatedSVD, saving .pkl)] ...
